...
class Strategy():
    def __init__(self, db, timer, coin1, coin2, config, name, mode, socket, save = True):
        ...
        self.coin1 = coin1
        self.coin2= coin2
        self.derivatives = [{'position' : 'close', 'coin2_balance' : 1, 'min_zoom' : {'c' : '>=', 'n' : 0}, 'total_investment' : 1}, {'position' : 'close', 'coin2_balance' : 1, 'min_zoom' : {'c' : '>=', 'n' : 0.02}, 'total_investment' : 1}]
        self.ready_to_use = 0
            ...
            for j in range(2):
                ...
                if (self.trade['type'] == trade_type):
                    ...
                    if (self.trade['price'] and self.stop_loss):
                        dif = ((dividend / (divisor * (1 + fee))) - 1) * int(leverage)#OK
                        ...
                        if (m > l): # Tendría ganancia con ese stop loss.
                            ...
                            stage = (((dividend / divisor) - 1) / fee) - 1#OK
                            ...
                            stage = ((divisor / (dividend * (1 - fee))) - 1) / fee#OK
                        ...
                        for d in self.derivatives:
                            ...
                            if (d['position'] != 'close'):
                                if (((c == '>') and (zoom > n)) or ((c == '>=') and (zoom >= n))):
                                    d['coin2_balance'] = d['coin2_balance'] * (1 + dif) * (1 - (fee2 * 0.5 * int(leverage)))
                                    d['position'] = 'long'
                                    if (self.trade['type'] == 'short'):
                                        d['position'] = 'short'
                                else:
